...
class PostService:
    ...
    def execute_publish(self, db: Session, post_id: int, user_id: Optional[int] = None) -> Post:
        ...
        if not meta_acc or not meta_acc.is_connected:
            # Check user's connected social accounts explicitly
            from app.models.social_account import SocialAccount
            user_sa = db.query(SocialAccount).filter(
            ...
            if not user_sa:
                ...
                db.commit()
                return post
            access_token = decrypt_token(user_sa.access_token)
            fb_page_id = user_sa.account_id if user_sa.platform == "facebook" else "sandbox"
            ig_user_id = user_sa.account_id if user_sa.platform == "instagram" else "sandbox"
        else:
            access_token = decrypt_token(meta_acc.access_token) or "sandbox_token"
            fb_page_id = meta_acc.facebook_page_id or "sandbox"
            ig_user_id = meta_acc.instagram_account_id or "sandbox"
        ...
        post.last_error = None if not (is_sandbox_fb or is_sandbox_ig) else "SANDBOX_MODE: Simulated publishing."
