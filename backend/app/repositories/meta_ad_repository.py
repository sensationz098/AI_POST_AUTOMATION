...
class MetaAdRepository:
    ...
    def sync_ads_for_user(
        ...
        """
        Synchronize list of raw Ads returned from Meta Graph API for a user.
        Matches each ad with its extracted creative mapping and performs idempotent upsert.
        """
        synced = []
        for ad_data in raw_ads:
            ...
            if not ad_id:
                continue
            mapped_info = mappings_map.get(ad_id, {})
            ad_rec = self.upsert(db, user_id, meta_ad_account_id, ad_data, mapped_info)
            synced.append(ad_rec)
        return synced
