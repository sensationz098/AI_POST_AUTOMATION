  ...
  return (
    <div className="space-y-5 select-none font-sans text-xs">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 pb-3 border-b border-slate-200 dark:border-slate-800/60">
        ...
        <div className="flex items-center space-x-1.5 bg-slate-100 dark:bg-slate-900/60 border border-slate-200 dark:border-slate-800 px-2.5 py-1 rounded text-[10px] font-mono text-emerald-600 dark:text-emerald-400">
      ...
      <div className="bg-white dark:bg-slate-900 rounded-lg overflow-hidden border border-slate-200 dark:border-slate-800/80 shadow-sm">
        <div className="px-3.5 py-2.5 bg-slate-50 dark:bg-slate-900/60 border-b border-slate-200 dark:border-slate-800/80 flex items-center justify-between">
          ...
          <span className="text-[10px] text-slate-500 dark:text-slate-400 font-mono">Showing latest 50 events</span>
        </div>

        <div className="overflow-x-auto">
          <table className="w-full text-left border-collapse text-xs">
            <thead>
              <tr className="bg-slate-50/50 dark:bg-slate-900/40 border-b border-slate-200 dark:border-slate-800/80 text-[10px] font-bold text-slate-600 dark:text-slate-400 uppercase tracking-wider">
            ...
            <tbody className="divide-y divide-slate-200 dark:divide-slate-800/60">
              {sampleLogs.map((log) => (
                <tr key={log.id} className="hover:bg-slate-50 dark:hover:bg-slate-800/40 transition-colors duration-150">
                  <td className="p-3">
                    <span className="px-2 py-0.5 rounded bg-indigo-50 dark:bg-indigo-950/60 border border-indigo-200 dark:border-indigo-800/60 text-indigo-700 dark:text-indigo-300 font-mono text-[10px] font-medium">
                      {log.action}
                    </span>
                  </td>
                  <td className="p-3 text-slate-800 dark:text-slate-200 font-medium font-mono text-[11px]">
                    {log.resource_type} {log.resource_id ? `#${log.resource_id}` : ''}
                  </td>
                  <td className="p-3 text-slate-600 dark:text-slate-400 font-mono text-[11px] max-w-sm truncate">
