...
export default function ConversationThread({
  ...
  return (
    <div className="bg-white dark:bg-slate-900/80 border border-slate-200 dark:border-slate-800/90 hover:border-slate-300 dark:hover:border-slate-700/80 rounded-2xl p-4 sm:p-5 space-y-4 transition-all shadow-sm">
      {/* Context Banner: Parent Meta Ad or Organic Post Source (if applicable) */}
      {showPostContext && (adCtx || postCtx) && (
        <div className="p-2.5 rounded-xl bg-slate-50 dark:bg-slate-950/90 border border-slate-200 dark:border-slate-800/80 text-xs flex items-center justify-between gap-3 text-slate-700 dark:text-slate-300">
          <div className="flex items-center space-x-2.5 min-w-0">
            {adCtx ? (
              <span className="px-2 py-0.5 rounded bg-purple-50 text-purple-700 border border-purple-200 dark:bg-purple-950/90 dark:text-purple-300 dark:border-purple-800/80 text-[10px] font-extrabold flex items-center space-x-1 flex-shrink-0">
                <Megaphone className="w-3 h-3 text-purple-600 dark:text-purple-400" />
                <span>Meta Ad</span>
              </span>
            ) : (
              <span className="px-2 py-0.5 rounded bg-indigo-50 text-indigo-700 border border-indigo-200 dark:bg-indigo-950/90 dark:text-indigo-300 dark:border-indigo-800/80 text-[10px] font-extrabold flex items-center space-x-1 flex-shrink-0">
            ...
            <div className="min-w-0 flex-1">
              ...
              {adCtx?.campaign_name && (
                <p className="text-[10px] text-slate-500 dark:text-slate-400 truncate">
                  Campaign: <span className="text-slate-700 dark:text-slate-300 font-medium">{adCtx.campaign_name}</span>
                </p>
              )}
            </div>
          </div>
      ...
      <div className="flex items-start justify-between gap-3">
        <div className="flex items-center space-x-3 min-w-0">
          ...
          <div className="min-w-0 space-y-0.5">
            <div className="flex items-center space-x-2 flex-wrap gap-y-1">
              ...
              {/* Status Pill */}
              {isReplied ? (
                <span className="px-2 py-0.5 rounded-full bg-emerald-50 text-emerald-700 border border-emerald-200 dark:bg-emerald-950/90 dark:text-emerald-300 dark:border-emerald-800/80 text-[10px] font-bold flex items-center space-x-1">
                  <span className="w-1.5 h-1.5 rounded-full bg-emerald-500 dark:bg-emerald-400"></span>
                  <span>Replied ({repliesList.length})</span>
                </span>
              ) : (
                <span className="px-2 py-0.5 rounded-full bg-amber-50 text-amber-700 border border-amber-200 dark:bg-amber-950/90 dark:text-amber-300 dark:border-amber-800/80 text-[10px] font-bold flex items-center space-x-1">
        ...
        <button
          ...
          disabled={deleting}
          className="p-1.5 rounded-lg text-slate-400 hover:text-rose-600 hover:bg-rose-50 dark:text-slate-500 dark:hover:text-rose-400 dark:hover:bg-rose-950/40 border border-transparent hover:border-rose-200 dark:hover:border-rose-800/50 transition flex-shrink-0"
      ...
      {/* Main Comment Text */}
      <div className="bg-slate-50 dark:bg-slate-950/70 p-4 rounded-xl border border-slate-200 dark:border-slate-800/80 text-sm text-slate-800 dark:text-slate-100 leading-relaxed font-sans whitespace-pre-wrap shadow-xs">
        {comment.comment_text || <em className="text-slate-400 dark:text-slate-500">No comment text</em>}
      </div>
      ...
      {repliesList.length > 0 && (
        <div className="pl-4 sm:pl-6 border-l-2 border-indigo-500/30 space-y-3 mt-4 pt-1">
          <div className="flex items-center space-x-2 text-[11px] font-bold text-indigo-600 dark:text-indigo-400 uppercase tracking-wider">
            ...
            return (
              <div
                ...
                className={`p-3.5 rounded-xl border transition-all text-xs space-y-2 shadow-xs ${
                  isOwnerReply
                    ? 'bg-indigo-50/70 dark:bg-indigo-950/40 border-indigo-200 dark:border-indigo-500/40 ring-1 ring-indigo-500/20'
              ...
              >
                    ...
                    <div className="min-w-0">
                      {isOwnerReply ? (
                        <div className="flex items-center space-x-1.5 font-bold text-indigo-700 dark:text-indigo-300">
                          ...
                          <CheckCircle2 className="w-3 h-3 text-emerald-600 dark:text-emerald-400 fill-emerald-400/20" />
                        </div>
                      ) : (
                        <div className="flex items-center space-x-1 font-semibold text-slate-800 dark:text-slate-200">
                          ...
                          <span className="text-[10px] text-slate-400 dark:text-slate-500 font-normal">via Meta</span>
                        </div>
                      )}
                    </div>
                  </div>

                  <div className="flex items-center space-x-1.5 text-[10px] text-slate-500 dark:text-slate-400 flex-shrink-0">
                    {isOwnerReply && (
                      <span className="px-1.5 py-0.2 rounded bg-emerald-50 text-emerald-700 border border-emerald-200 dark:bg-emerald-950/80 dark:text-emerald-300 dark:border-emerald-800/60 font-semibold text-[9px]">
                        Sent
                      </span>
                    )}
                    <span>{replyDate}</span>
                ...
                {/* Reply Body */}
                <p className="text-slate-800 dark:text-slate-100 font-sans leading-relaxed pl-8 whitespace-pre-wrap">
                  {reply.message}
                </p>
              </div>
            );
      ...
      {isReplying ? (
        ...
        <div className="flex justify-end pt-1">
          <button
            ...
            className="px-4 py-2 rounded-xl bg-indigo-50 hover:bg-indigo-100 text-indigo-700 border border-indigo-200 dark:bg-indigo-600/20 dark:hover:bg-indigo-600/30 dark:text-indigo-300 dark:border-indigo-500/40 text-xs font-semibold transition flex items-center space-x-1.5 shadow-sm"
          >
