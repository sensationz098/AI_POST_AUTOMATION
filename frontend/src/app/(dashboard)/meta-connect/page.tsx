...
import {
  ...
  Search, ChevronLeft, Filter, X, MessageSquare, Youtube, UploadCloud
} from 'lucide-react';
                ...
                <div key={acc.id} className="bg-slate-900/60 rounded-xl border border-slate-800 overflow-hidden">
                  ...
                  {adCommentSyncSuccess[acctId] && (
                    <div className="mx-4 mt-3 bg-indigo-950/40 border border-indigo-800/60 rounded-lg p-2.5 text-xs text-indigo-300 flex items-center justify-between gap-2">
                      ...
                      <Link
                        href={`/comments?tab=ads&adAccountId=${acctId}`}
                        className="px-2.5 py-1 rounded bg-indigo-600 hover:bg-indigo-500 text-white font-semibold text-[11px] transition flex items-center space-x-1 flex-shrink-0 shadow-sm"
                      >
                        <span>View Ad Comments</span>
