...
export default function PostPickerModal({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-950/60 dark:bg-black/75 backdrop-blur-sm animate-fadeIn">
      <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-3xl max-w-3xl w-full max-h-[85vh] flex flex-col shadow-2xl overflow-hidden relative">
        {/* Modal Header */}
        <div className="p-5 border-b border-slate-200 dark:border-slate-800/80 flex items-center justify-between">
          ...
          <div className="flex items-center space-x-2">
            <button
              ...
              className="text-slate-400 hover:text-slate-700 dark:hover:text-slate-200 p-1.5 rounded-xl hover:bg-slate-100 dark:hover:bg-slate-800 transition disabled:opacity-50"
            >
        ...
        {/* Search Bar */}
        <div className="p-4 border-b border-slate-200 dark:border-slate-800/60 bg-slate-50/60 dark:bg-slate-950/40">
          <div className="relative">
            ...
            <input
              ...
              className="w-full bg-white dark:bg-slate-900 border border-slate-300 dark:border-slate-800 rounded-xl pl-10 pr-4 py-2 text-xs text-slate-900 dark:text-slate-200 placeholder-slate-400 dark:placeholder-slate-500 focus:outline-none focus:border-indigo-500 transition shadow-inner"
            />
          </div>
        </div>
        ...
        <div className="flex-1 overflow-y-auto p-4 space-y-3">
          {isLoading ? (
            ...
            <div className="p-6 rounded-2xl bg-rose-50 dark:bg-rose-500/10 border border-rose-200 dark:border-rose-500/20 text-rose-800 dark:text-rose-300 text-xs flex items-center space-x-3">
              ...
              <div className="flex-1">
                ...
                <button
                  type="button"
                  onClick={fetchPlatformPosts}
                  className="mt-2 inline-flex items-center space-x-1 text-xs text-rose-600 dark:text-rose-200 underline font-medium"
                >
            ...
            <div className="py-16 text-center text-slate-500 dark:text-slate-400 space-y-2">
              <ImageIcon className="w-10 h-10 mx-auto text-slate-400 dark:text-slate-600" />
              <p className="text-xs font-semibold text-slate-900 dark:text-slate-300">No posts found on this account</p>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
              {filteredPosts.map((post) => {
                ...
                return (
                  ...
                  >
                    {/* Media Thumbnail */}
                    <div className="w-16 h-16 rounded-xl bg-slate-100 dark:bg-slate-900 border border-slate-200 dark:border-slate-800 overflow-hidden flex-shrink-0 flex items-center justify-center relative">
                      {mediaUrl ? (
                        <img
                          src={mediaUrl}
                          alt="Post thumbnail"
                    ...
                    <div className="flex-1 min-w-0 space-y-1">
                      <div className="flex items-center justify-between gap-1">
                        ...
                        {dateStr && (
                          <span className="text-[10px] text-slate-400 dark:text-slate-500 flex items-center space-x-1 flex-shrink-0">
                      ...
                      <div className="pt-1 flex flex-col space-y-0.5 text-[10px]">
                        <span className="font-mono text-slate-500 dark:text-slate-400 truncate">
                          ...
                          <span className="text-slate-800 dark:text-slate-200 font-semibold">{post.id}</span>
                        </span>
        ...
        {/* Modal Footer */}
        <div className="p-4 border-t border-slate-200 dark:border-slate-800/80 bg-slate-50/80 dark:bg-slate-950/60 flex items-center justify-between text-xs">
          ...
          <button
            type="button"
            onClick={onClose}
            className="px-4 py-2 rounded-xl bg-slate-100 hover:bg-slate-200 dark:bg-slate-800 dark:hover:bg-slate-700 text-slate-700 dark:text-slate-200 font-semibold transition border border-slate-300 dark:border-transparent shadow-sm"
          >
            Close
          </button>
        </div>
