  ...
  return (
    <div className="space-y-6 pb-12 animate-fadeIn font-sans">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        ...
        <div className="flex items-center space-x-3">
          <button
            ...
            onClick={fetchData}
            disabled={isLoading}
            className="p-2.5 rounded-xl bg-white dark:bg-slate-900/80 hover:bg-slate-100 dark:hover:bg-slate-800 border border-slate-200 dark:border-slate-800 text-slate-700 dark:text-slate-300 transition shadow-sm"
          ...
          <button
            ...
            className="px-5 py-2.5 rounded-xl bg-indigo-600 hover:bg-indigo-500 text-white font-bold text-xs transition shadow-lg shadow-indigo-600/20 flex items-center space-x-2 group focus:outline-none focus:ring-2 focus:ring-indigo-500/50"
          >
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
        {/* Total */}
        <div className="p-4 rounded-2xl bg-white dark:bg-slate-900/60 border border-slate-200 dark:border-slate-800/80 shadow-sm flex items-center space-x-3">
          ...
          <div>
            ...
            <p className="text-lg font-black text-slate-900 dark:text-slate-100">{metrics.total}</p>
          </div>
        </div>
        ...
        <div className="p-4 rounded-2xl bg-white dark:bg-slate-900/60 border border-slate-200 dark:border-slate-800/80 shadow-sm flex items-center space-x-3">
          ...
          <div>
            <span className="text-[10px] uppercase font-bold text-slate-500 dark:text-slate-400 tracking-wider">Active</span>
            <p className="text-lg font-black text-emerald-600 dark:text-emerald-400">{metrics.active}</p>
          </div>
        </div>

        {/* Paused */}
        <div className="p-4 rounded-2xl bg-white dark:bg-slate-900/60 border border-slate-200 dark:border-slate-800/80 shadow-sm flex items-center space-x-3">
          <div className="w-10 h-10 rounded-xl bg-amber-50 dark:bg-amber-500/10 flex items-center justify-center text-amber-600 dark:text-amber-400 flex-shrink-0">
            <Pause className="w-5 h-5" />
          </div>
          <div>
            <span className="text-[10px] uppercase font-bold text-slate-500 dark:text-slate-400 tracking-wider">Paused</span>
            <p className="text-lg font-black text-amber-600 dark:text-amber-400">{metrics.paused}</p>
          </div>
        </div>

        {/* Drafts */}
        <div className="p-4 rounded-2xl bg-white dark:bg-slate-900/60 border border-slate-200 dark:border-slate-800/80 shadow-sm flex items-center space-x-3">
          <div className="w-10 h-10 rounded-xl bg-slate-100 dark:bg-slate-800 flex items-center justify-center text-slate-600 dark:text-slate-400 flex-shrink-0">
            <Layers className="w-5 h-5" />
          </div>
          <div>
            <span className="text-[10px] uppercase font-bold text-slate-500 dark:text-slate-400 tracking-wider">Drafts</span>
            <p className="text-lg font-black text-slate-800 dark:text-slate-300">{metrics.drafts}</p>
          </div>
        </div>
      </div>

      {/* 3. Search & Filter Bar */}
      <div className="p-4 rounded-2xl bg-white dark:bg-slate-900/80 border border-slate-200 dark:border-slate-800/90 shadow-sm flex flex-col md:flex-row items-stretch md:items-center justify-between gap-3">
        {/* Search Input */}
        <div className="relative flex-1">
          ...
          <input
            ...
            className="w-full bg-slate-50 dark:bg-slate-950 border border-slate-300 dark:border-slate-800 rounded-xl pl-10 pr-4 py-2 text-xs text-slate-900 dark:text-slate-200 placeholder-slate-400 dark:placeholder-slate-500 focus:outline-none focus:border-indigo-500 transition shadow-inner"
          />
        </div>
        ...
        <div className="flex items-center space-x-2.5 flex-wrap gap-y-2">
          ...
          <div className="relative">
            <select
              ...
              className="bg-slate-50 dark:bg-slate-950 border border-slate-300 dark:border-slate-800 text-slate-800 dark:text-slate-200 text-xs font-semibold rounded-xl pl-3 pr-8 py-2 outline-none focus:border-indigo-500 appearance-none cursor-pointer shadow-sm"
            >
      ...
      {isLoading ? (
        ...
        <div className="p-6 rounded-2xl bg-rose-50 dark:bg-rose-500/10 border border-rose-200 dark:border-rose-500/20 text-rose-800 dark:text-rose-300 text-xs flex items-center justify-between">
          <div className="flex items-center space-x-3">
        ...
        <div className="py-16 text-center text-slate-500 dark:text-slate-400 space-y-2 bg-white dark:bg-slate-900/40 border border-slate-200 dark:border-slate-800/60 rounded-3xl p-8 shadow-sm">
          <Filter className="w-8 h-8 mx-auto text-slate-400 dark:text-slate-600" />
          <p className="text-xs font-bold text-slate-900 dark:text-slate-200">No matching automations found</p>
